...
export async function sendCampaignPush(_prev: CampaignPushState, formData: FormData): Promise<CampaignPushState> {
  ...
  if (!(await isPushConfigured())) return { error: "El push web todavía no está configurado." };
...
export async function sendTestPush(): Promise<TestPushState> {
  ...
  if (!(await isPushConfigured())) return { devices, fallback, error: "El centro interno funciona, pero el servicio push no está configurado." };
