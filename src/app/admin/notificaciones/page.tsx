...
type GatewayStatus = { configured?: boolean };
type GatewayStats = { ok?: boolean; totalDevices?: number; usersWithPush?: number; recentDevices?: number; ownDevices?: DeviceRow[] };
...
export default async function AdminNotificationsPage() {
  ...
  let stats = { totalDevices: 0, usersWithPush: 0, recentDevices: 0 };
  let mine: DeviceRow[] = [];
  const base = process.env.NEXT_PUBLIC_SUPABASE_URL;
  if (base && session?.access_token) {
    try {
      const [statusResponse, statsResponse] = await Promise.all([
        fetch(`${base}/functions/v1/push-gateway`, { cache: "no-store" }),
        fetch(`${base}/functions/v1/push-gateway`, {
          method: "POST",
      ...
      const statusData = await statusResponse.json().catch(() => ({})) as GatewayStatus;
      const statsData = await statsResponse.json().catch(() => ({})) as GatewayStats;
      configured = statusResponse.ok && statusData.configured === true;
      if (statsResponse.ok && statsData.ok) {
        stats = { totalDevices: statsData.totalDevices ?? 0, usersWithPush: statsData.usersWithPush ?? 0, recentDevices: statsData.recentDevices ?? 0 };
        mine = statsData.ownDevices ?? [];
      }
    } catch {
      configured = false;
    }
  ...
  return <div className="max-w-3xl space-y-7">
    ...
    {!configured ? <div className="rounded-2xl border border-cirio-200 bg-cirio-50 p-4 text-sm text-anil-900">El centro interno funciona, pero el gateway push no respondió como configurado.</div> : null}
