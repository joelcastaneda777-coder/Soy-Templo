  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (file?.size) {
        ...
        const ext = safeFileName(file.name) || "episodio.mp3";
        uploadedPath = `${programId}/${Date.now()}-${crypto.randomUUID()}-${ext}`;
      ...
      if (uploadedPath) await supabase.storage.from("radio-archive").remove([uploadedPath]).catch(() => undefined);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-[var(--radius-card)] border border-manta bg-white p-5 shadow-sm">
      ...
      <div className="rounded-2xl border border-dashed border-anil-200 bg-anil-50/40 p-4">
        ...
        {pending && progress > 0 ? <div className="mt-3"><div className="h-2 overflow-hidden rounded-full bg-anil-100"><div className="h-full rounded-full bg-anil-600 transition-[width]" style={{ width: `${progress}%` }} /></div><p className="mt-1 text-right text-xs font-semibold text-anil-700">{progress}%</p></div> : null}
      </div>
