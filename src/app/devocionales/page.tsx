  ...
  return (
    <div className="space-y-5">
      ...
      <form role="search" className="flex gap-2">
        ...
        />
        <button className="rounded-full bg-[#063547] px-5 font-semibold text-white hover:bg-[#084B53]">Buscar</button>
      </form>
            ...
            return (
              <li key={d.slug} className="stagger-item" style={{ animationDelay: `${i * 40}ms` }}>
                <Link href={`/devocionales/${d.slug}`}>
                  <Card className="transition-colors hover:border-[#3a7f82]">
                    <div className="flex items-center gap-2 text-xs font-medium text-tinta-suave">
                      <span>{formatDate(d.publish_at!)}</span>
                      {isToday ? (
                        <span className="rounded-full bg-[#d8ecea] px-2 py-0.5 font-semibold text-[#063547]">Hoy</span>
                      ) : null}
                    </div>
                    <h2 className="mt-1 font-display text-xl font-semibold">{d.title}</h2>
                    <p className="mt-1 text-sm text-[#0d5d63]">{d.bible_reading}</p>
                  </Card>
                </Link>
              </li>
            );
