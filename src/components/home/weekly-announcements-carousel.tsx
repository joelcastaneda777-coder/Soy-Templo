        ...
        >
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.id}
                className={`relative w-[82vw] max-w-[430px] shrink-0 snap-center overflow-hidden rounded-[1.8rem] border border-white/60 bg-white shadow-[0_14px_38px_rgba(11,43,38,0.12)] transition duration-300 dark:border-white/10 dark:bg-manta ${active ? "scale-100 opacity-100" : "scale-[0.96] opacity-70"}`}
              >
                {item.imageUrl ? (
                ...
                <div className="p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {item.featured ? <span className="text-xs font-semibold text-cirio-600">★ Destacado</span> : null}
                  </div>
                  <h3 className="mt-3 font-display text-xl font-semibold leading-tight text-anil-900 dark:text-tinta">{item.title}</h3>
                  <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-tinta-suave">{item.description}</p>
                  <Link href={href} className="mt-4 inline-flex text-sm font-semibold text-anil-600">
        ...
        {items.length > 1 ? (
          <>
            <button
              ...
              className="absolute left-2 top-1/2 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-manta bg-papel/90 text-anil-800 shadow-md backdrop-blur md:flex"
            >
              ←
            </button>
            <button
              ...
              className="absolute right-2 top-1/2 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-manta bg-papel/90 text-anil-800 shadow-md backdrop-blur md:flex"
            >
              →
            </button>
          </>
